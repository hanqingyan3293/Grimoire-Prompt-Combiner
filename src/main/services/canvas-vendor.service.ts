...
function rowToProject(row: unknown[]): CanvasVendorProjectRecord {
  ...
  return normalizeProject({ ...parsed, id: String(row[0]), title: String(row[1]), createdAt: String(row[3]), updatedAt: String(row[4]) }, String(row[0]))
}

export function listCanvasVendorProjects(): Array<Pick<CanvasVendorProjectRecord, 'id' | 'title' | 'createdAt' | 'updatedAt'>> {
  const result = getDatabase().exec('SELECT id, title, data_json, created_at, updated_at FROM canvas_vendor_projects ORDER BY updated_at DESC')
...
export function getCanvasVendorProject(id: string): CanvasVendorProjectRecord | null {
  const result = getDatabase().exec('SELECT id, title, data_json, created_at, updated_at FROM canvas_vendor_projects WHERE id=?', [id])
...
export function createCanvasVendorProject(title = '未命名画布', requestedId?: string): CanvasVendorProjectRecord {
  ...
  getDatabase().run('INSERT INTO canvas_vendor_projects (id, title, data_json, created_at, updated_at) VALUES (?,?,?,?,?)', [project.id, project.title, JSON.stringify(project), project.createdAt, project.updatedAt])
  saveDatabase()
  return project
}
...
export function saveCanvasVendorProject(id: string, value: unknown): CanvasVendorProjectRecord {
  ...
  getDatabase().run('UPDATE canvas_vendor_projects SET title=?, data_json=?, updated_at=? WHERE id=?', [project.title, JSON.stringify(project), project.updatedAt, id])
  saveDatabase()
  return project
}
