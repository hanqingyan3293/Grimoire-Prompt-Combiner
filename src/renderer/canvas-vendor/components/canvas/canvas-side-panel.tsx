...
export function CanvasSidePanel({ nodes, selectedNodeIds, onFocusNode, onPreviewNode, onInsertAsset }: Props) {
    ...
    useEffect(() => {
        const refresh = () => void useAssetStore.getState().refresh().catch(console.error);
        window.addEventListener('grimoire:refresh', refresh);
        return () => window.removeEventListener('grimoire:refresh', refresh);
    }, []);
