...
export function FloatingPreview({ open, title, onClose, children, footer }: { open: boolean; title: string; onClose: () => void; children: ReactNode; footer?: ReactNode }) {
  ...
  return <div className="fixed inset-0 z-[1200] bg-black/45 backdrop-blur-[2px]" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="absolute flex min-h-0 min-w-0 flex-col overflow-visible rounded-xl border border-[var(--color-border-strong)] bg-[var(--color-bg-primary)] shadow-[0_24px_80px_rgba(0,0,0,.42)] outline-none" style={{ left: rect.left, top: rect.top, width: rect.width, height: rect.height }}>
      <header onPointerDown={startDrag} className="flex h-12 shrink-0 cursor-move items-center justify-between border-b-2 border-[var(--color-border-strong)] bg-[var(--color-bg-secondary)] px-4 font-semibold">
        <span className="truncate">{title}</span><button type="button" onPointerDown={(e) => e.stopPropagation()} onClick={onClose} className="grid size-8 place-items-center rounded-lg outline-none hover:bg-black/10 focus:outline-none dark:hover:bg-white/10"><X size={18} /></button>
      </header>
