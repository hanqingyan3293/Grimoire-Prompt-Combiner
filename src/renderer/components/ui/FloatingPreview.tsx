...
export function FloatingPreview({ open, title, children, onClose, footer }: { open: boolean; title: string; children: ReactNode; onClose: () => void; footer?: ReactNode }) {
  ...
  return <div className='fixed inset-0 z-[1200] bg-black/50 backdrop-blur-[2px]' onPointerDown={event => { if (event.target === event.currentTarget) onClose() }}>
    <section className='absolute flex min-h-0 min-w-0 flex-col overflow-visible rounded-xl border border-[var(--color-border-strong)] bg-[var(--color-bg-primary)] shadow-[0_24px_80px_rgba(0,0,0,.45)] outline-none' style={rect}>
      ...
      <div className='min-h-0 min-w-0 flex-1 overflow-auto p-4'>{children}</div>{footer ? <footer className='shrink-0 border-t-2 border-[var(--color-border-strong)] bg-[var(--color-bg-secondary)] p-3'>{footer}</footer> : null}
